...
static ClaylandSeat *
clayland_pointer_get_seat (ClaylandPointer *pointer)
{
  ClaylandSeat *seat = wl_container_of (pointer, seat, pointer);
                    ...
                    wl_fixed_t y)
{
  ...
  clayland_pointer_set_focus (pointer, surface, x, y);
}

static void
default_grab_motion (ClaylandPointerGrab *grab,
...
static void
default_grab_button (ClaylandPointerGrab *grab,
                     uint32_t time, uint32_t button, uint32_t state_w)
{
  ...
  if (pointer->button_count == 0 && state == WL_POINTER_BUTTON_STATE_RELEASED)
    clayland_pointer_set_focus (pointer, pointer->current,
                                pointer->current_x, pointer->current_y);
}

static const ClaylandPointerGrabInterface default_pointer_grab_interface = {
  default_grab_focus,
  default_grab_motion,
  default_grab_button
};
                            ...
                            wl_fixed_t sx, wl_fixed_t sy)
{
  ...
  ClaylandKeyboard *kbd = &seat->keyboard;
